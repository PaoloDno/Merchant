...
export const adminGetProfiles = createAsyncThunk(
  ...
  async ({ firstname = "", lastname = "", page = 1, limit = 15, sortBy = "createdAt", sortOrder = "desc" }, thunkAPI) => {
    try {
      ...
      thunkAPI.dispatch(setError({ message: error.response?.data?.message || "An error occurred", status: error.response?.status }));
...
export const adminGetStores = createAsyncThunk(
  ...
  async ({ storeName = "", rating, isVerified, page = 1, limit = 15, sortBy = "createdAt", sortOrder = "desc" }, thunkAPI) => {
    try {
...
export const adminGetProducts = createAsyncThunk(
  ...
  async ({ productName = "", category = "", subcategory = "", hot = false, isNew = false, bestSelling = false, page = 1, limit = 15, sortBy = "createdAt", sortOrder = "desc" }, thunkAPI) => {
    try {
...
export const adminGetCategories = createAsyncThunk(
  ...
  async ({ name = "", page = 1, limit = 15, sortBy = "createdAt", sortOrder = "desc" }, thunkAPI) => {
    try {
